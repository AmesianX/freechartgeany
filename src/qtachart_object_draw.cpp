...
QTACObject::draw ()
{
  ...
  valueSet (); // calculate the value set 
  ...
  if (type != QTACHART_OBJ_SUBCHART) // object is not a subchart
  {
    ...
    for (qint32 counter = *core->startbar, i = 0;
         counter < max && counter < valueset->size ();
         counter ++, i ++)
    {
      qreal x;
         ...
         counter ++)
    {
	  ...
	  {  	
        ...
        if (parentObject == NULL || 
           (parentObject != NULL && parentObject->type == QTACHART_OBJ_CURVE))
    ...
    if (type == QTACHART_OBJ_CURVE || type == QTACHART_OBJ_DOT)
    {
	  ...
	  if (*core->startbar == 0 && dataset->size () > period)
	  {
		if (parentObject == NULL)
		{
		  ...
		  if (rectf.width () + (X[0] + 5) < core->chartrightmost)
		  {	
		    title->setVisible (true);
		    title->setDefaultTextColor (forecolor);
		    title->setPos (X[0] + 5, Y[0] - 12); 
		  }  
		}  
      }	
  ...
  else // object is a subchart
  {
    ...
    for (quint32 counter = 0; counter < nobjects &&
                              core->Object[counter] != this; 
                              counter ++)
    {
	  objcursor = core->Object[counter];
...
QTACObject::drawText ()
{
  ...
  for (int counter = 0; counter < core->nbars_on_chart; counter ++)
  {
    if ( counter + *core->startbar < core->HLOC->size ()) 	  
      if (trailerCandleText == (*core->HLOC)[counter + *core->startbar].Text)
	    found = counter;    
  }	
...
QTACObject::drawTLine ()
{
  ...
  {
    if ( counter + *core->startbar < core->HLOC->size ()) 	  
      if (Edge[0]->trailerCandleText == (*core->HLOC)[counter + *core->startbar].Text)
	    found = counter;    
  }	
  ...
  {
    if ( counter + *core->startbar < core->HLOC->size ()) 	  
      if (Edge[1]->trailerCandleText == (*core->HLOC)[counter + *core->startbar].Text)
	    found = counter;    
  }	
