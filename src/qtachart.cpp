  ...
  QWidget (parent), ui (new Ui::QTAChart)
{
  ...
  core->height = height ();
  core->width = width ();
  core->chartrightmost = (core->width - core->right_border_width);
  ...
  core->chart->setCacheMode (QGraphicsView::CacheNone);
  ...
  // help button
  core->helpicon =
    QIcon (QString::fromLocal8Bit (":/png/images/icons/PNG/shortcuts.png"));
  core->helpBtn->setIcon (core->helpicon);
  core->helpBtn->setIconSize (QSize (32, 32));
  core->helpBtn->setStyleSheet ("background: transparent;color: white;font: 11px;");
  ...
  core->prxhelpBtn->setGeometry (QRectF(0, 90, 32, 32));
  ...
  core->propScr->setChartStyle (core->chart_style);
  core->propScr->setLinearScale (core->linear);
...
QTAChart::goBack (void)
{
  ...
  core->chart_style = core->propScr->ChartStyle ();
  core->linecolor = core->propScr->lineColor ();
  core->barcolor = core->propScr->barColor ();
  core->linear = core->propScr->LinearScale ();
...
QTAChart::keyPressEvent (QKeyEvent * event)
{
  ...
  if (event->modifiers () & Qt::AltModifier)
  {
    ...
    {
	  core->deleteITEMS ();		
      core->propScr->setChartStyle (QTACHART_LINE);
      core->chart_style = core->propScr->ChartStyle ();
      goto EventEndLbl;
    }
    ...
    {
	  core->deleteITEMS ();	
      core->propScr->setChartStyle (QTACHART_CANDLE);
      core->chart_style = core->propScr->ChartStyle ();
      goto EventEndLbl;
    }
    ...
    {
	  core->deleteITEMS ();		
      core->propScr->setChartStyle (QTACHART_HEIKINASHI);
      core->chart_style = core->propScr->ChartStyle ();
      goto EventEndLbl;
    }
    ...
    {
	  core->deleteITEMS ();		
      core->propScr->setChartStyle (QTACHART_BAR);
      core->chart_style = core->propScr->ChartStyle ();
      goto EventEndLbl;
    }
    ...
    if (event->key () == Qt::Key_X)
    {
      if (core->propScr->LinearScale ())
        core->propScr->setLinearScale (false);
      else
        core->propScr->setLinearScale (true);
      core->linear = core->propScr->LinearScale ();
      goto EventEndLbl;
    }
  }
...
QTAChart::expandBtn_clicked (void)
{
  ...
  mainwindow = ((MainWindow *) (this->parentWidget ()->parentWidget ()->
